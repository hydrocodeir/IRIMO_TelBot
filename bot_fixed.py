...
# ---------------- Safe edit helpers ----------------
# Telegram returns HTTP 400 "message is not modified" when we try to edit a message
# with the exact same text/markup. These wrappers ignore that specific case.
...
def callback_handler(call):
    ...
    if "_page|" in call.data:
        ...
        if prefix.startswith("region"):
            ...
            if str(user_id) == str(ADMIN_ID):
                markup.add(InlineKeyboardButton("📊 Admin Report", callback_data="admin_report"))
            safe_edit_message_reply_markup(bot, call.message.chat.id, call.message.message_id, reply_markup=markup)
    ...
    if call.data == "back_to_provinces":
        ...
        if str(user_id) == str(ADMIN_ID):
            markup.add(InlineKeyboardButton("📊 Admin Report", callback_data="admin_report"))
        safe_edit_message_text(bot, "🔙 Back to province selection:", call.message.chat.id, call.message.message_id, reply_markup=markup)
        return
